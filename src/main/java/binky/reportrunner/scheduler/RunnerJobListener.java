...
public class RunnerJobListener implements JobListener {
	...
	public void jobExecutionVetoed(JobExecutionContext ctx) {
		auditService.logAuditEvent(this.getClass().getName(), false, 0, "{"
				+ ctx.getJobDetail().getName() + "},{"
				+ ctx.getJobDetail().getGroup() + "}", "jobExecutionVetoed","Job Execution was vetoed by something");
	...
	public void jobWasExecuted(JobExecutionContext ctx, JobExecutionException ex) {
		...
		if (!success) errorText=ex.getMessage();
		
		if (ctx.getJobDetail().getJobClass().equals(RunnerEngine.class)) {

			auditService.logAuditEvent(this.getClass().getName(), success, 0, "{"
					+ ctx.getJobDetail().getName() + "},{"
					+ ctx.getJobDetail().getGroup() + "}", "jobWasExecuted",errorText);
		} else if (ctx.getJobDetail().getJobClass()
				.equals(AlertProcessor.class)) {
			// get the real name of the job that fired as we use an
			// autogenerated ID
			RunnerDashboardItem item = dashboardService.getItem(Integer
					.parseInt(jobName));
			StringBuilder itemName = new StringBuilder();
			itemName.append(item.getItemName());
			itemName.append(' ');
			itemName.append('(');
			itemName.append(jobName);
			itemName.append(')');
			auditService.logAuditEvent(this.getClass().getName(), success, 0, "{"
					+ itemName + "},{" + item.getGroup().getGroupName() + "}",
					"jobWasExecuted",errorText);
		}
