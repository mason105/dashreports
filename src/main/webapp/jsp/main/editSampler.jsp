			...
			<sj:tabbedpanel id="report" animate="true"
				...
				<div id="detailsDiv">
					<div class="formGroup">
						...
						<s:textfield label="Item Name" size="64" value="%{item.itemName}"
							name="item.itemName" cssClass="textbox" required="true">
						</s:textfield>
						<s:hidden name="item.group.groupName" value="%{groupName}" />
						<s:select label="Select Data Source" name="dataSourceName"
							value="%{item.datasource.dataSourceName}"
							list="runnerDataSources" cssClass="textbox"
							onchange="dojo.event.topic.publish('refresh_fields');">
						</s:select>

						<s:textarea label="Query" cols="30" rows="20"
							value="%{item.alertQuery}" name="itemQuery" cssClass="textbox"
							required="true"
							onchange="dojo.event.topic.publish('refresh_fields');">
						</s:textarea>

						<s:url id="validateUrl" action="validateItemQuery" />
						<sx:div showLoadingText="true" loadingText="Testing query..."
						...
						<s:select label="Sampling Interval" name="item.interval"
							list="intervals" listKey="name" listValue="displayName"
							cssClass="textbox"></s:select>

						<s:select label="Sampling Window" name="item.window"
							list="windows"
							tooltip="this is the time the data is retained for"
							listKey="name" listValue="displayName" cssClass="textbox"></s:select>

						<s:url id="fieldsUrl" action="getSamplerColumns" />
						<sx:div showLoadingText="true" loadingText="Getting fields..."
							id="refreshFields" href="%{fieldsUrl}" theme="ajax"
							listenTopics="refresh_fields" formId="saveSampler">
						</sx:div>
				...
				<div id="layoutDiv">
					<div class="formGroup">
						...
						<div style="float: left; padding-right: 10px;">
							<sj:spinner name="item.displayColumn" id="item.displayColumn"
								min="1" max="50" step="1" value="%{item.displayColumn}"
								label="Display Column" />
						</div>
						<div>
							<sj:spinner name="item.displayRow" id="item.displayRow" min="1"
								max="50" step="1" value="%{item.displayRow}" label="Display Row" />
						</div>
				...
				<div id="chartDiv">
					<div class="formGroup">
						...
						<s:select label="Orientation" name="item.orientation"
					...
					'#FAF8CC':'Light Goldenrod Yellow'}"
						...
						<s:textfield label="Y-Axis Label" size="64"
							value="%{item.yAxisLabel}" name="item.yAxisLabel"
							cssClass="textbox">
						</s:textfield>
