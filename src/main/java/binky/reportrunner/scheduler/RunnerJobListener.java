	...
	public void jobWasExecuted(JobExecutionContext ctx, JobExecutionException ex) {
		...
		if (ctx.getJobDetail().getJobClass().equals(RunnerEngine.class)) {
			...
			auditService.logAuditEvent(Module.REPORT_SCHEDULER, message, "", success, ctx.getJobRunTime(), jobName, groupName);
				...
				.equals(AlertProcessor.class)) {
			//get the real name of the job that fired as we use an autogenerated ID
			RunnerDashboardItem item = dashboardService.getItem(Integer.parseInt(jobName));			
			...
			itemName.append(')');
			auditService.logAuditEvent(Module.DASHBOARD_SCHEDULER, message, "", success, ctx.getJobRunTime(), itemName.toString(), item.getGroup().getGroupName());			
		}
